...
Вы должны выбрать нужный сервис и сохнарить его `code` и `fields['key']`, по этому для `payment_card_uah` это будет `code = payment_card_uah` и `fields['key'] = card_number`.
...
#### `fields`
Содержит имя поля и значение для вывода. Имя поля - это ключ, который вы получили из `/payout_prerequest` (`card_number` из примера выше), а значение должно быть номером кредитной карты, поэтому этот параметр должен быть похож на:
```bash
{
...
#### `gateway` 
Имя сервиса, который будет использован для вывода, это имя сервиса из `/payout_prerequest` (`payment_card_uah` из примера выше).
...
Вы можете совершить вывод используя предварительно сохраненный список средств. В этом случае вам не нужно получать информацию по `/payout_prerequest`. \
Просто выполните запрос [Создание заявки на вывод] с `fund_source_id` вместо `fields`:
