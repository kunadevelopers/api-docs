...
Ввод и вывод фиата на Kuna осуществляется через сервис `pay.kuna.io`. В Kuna существует 3 фиатные валюты: UAH, USD и RUB. В этом разделе описана процедура для автоматизации процесса ввода и вывода средств. Все методы в Kuna приватные, а значит потребуют подписи.
...
В ответ вы получите `deposit_id`. Его нужно использовать в поле `reference_id` в запросе `POST https://pay.kuna.io/public-api/payment-invoices`.
...
```bash
{ 
  "action": "https://pay.kuna.io/public-api/payment-invoices/process", 
  "params": [
